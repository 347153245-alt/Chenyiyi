...
import { Plus, Trash2, Printer, ChevronUp, ChevronDown, RotateCcw, Save, MapPin, Clock, Info, DollarSign, ZoomIn, ZoomOut } from 'lucide-react';
...
const App: React.FC = () => {
  const STORAGE_KEY = 'sttm-agenda-v42-final';
  ...
  const handleSaveData = () => {
    ...
    alert('保存成功！\n注意：保存内容仅存于当前浏览器，换台电脑或清除缓存后将无法查看。');
  };

  const handlePrint = () => {
    alert('打印提示：\n为了确保议程能够完美显示在一张 A4 纸内，请在打印预览设置中将“缩放”调整为 97%。');
    window.print();
  };
        ...
        >
            ...
            <div className="flex-grow flex flex-col items-end">
              ...
              <div className="flex items-center gap-2 w-full justify-end mt-1">
                ...
                <div className="flex items-baseline gap-1 bg-[#772432] px-2 py-0.5 rounded shadow-sm shrink-0">
                  ...
                  <input className="text-lg font-black text-[#F2DF74] w-10 bg-transparent outline-none leading-none text-center" value={info.meetingNumber} onChange={e => setInfo({...info, meetingNumber: e.target.value})} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1 mb-2 shrink-0">
            <div className="flex items-baseline gap-2">
              ...
              <input className="text-xl font-black text-[#772432] bg-transparent outline-none flex-grow border-b border-dashed border-gray-200 focus:border-[#772432] px-1 italic leading-none pb-0.5" value={info.theme} onChange={e => setInfo({...info, theme: e.target.value})} placeholder="Theme..." />
            </div>
          ...
          <div className="flex-grow flex gap-4 overflow-hidden min-h-0">
            ...
            {/* LEFT MAIN AGENDA (58%) - Adjusted to give Role more room */}
                ...
                {computedAgenda.map((item, index) => (
                  <React.Fragment key={item.id}>
                    ...
                    <div className={`group relative transition-all duration-150 ${item.isSectionHeader ? 'my-0.5' : ''} shrink-0`}>
                      <div className="absolute -left-8 top-1/2 -translate-y-1/2 no-print flex flex-col gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity bg-white shadow rounded p-0.5 border border-gray-100 z-40">
                      ...
                      {item.isSectionHeader ? (
                        ...
                        <div className="flex items-start border-b border-gray-50 py-0.5 hover:bg-amber-50/10 transition-all overflow-hidden">
                          ...
                          <div className="flex-grow px-2 overflow-hidden min-w-0">
                            <textarea rows={1} className="w-full bg-transparent outline-none font-bold text-gray-800 text-[10px] resize-none overflow-hidden block leading-tight pb-0.5" value={item.activity} onInput={e => handleAutoHeight(e.target as HTMLTextAreaElement)} onChange={e => updateAgendaItem(item.id, 'activity', e.target.value)} />
                          </div>
            ...
            {/* RIGHT SIDEBAR (42%) - Increased gap between modules */}
            <div className="w-[42%] flex flex-col gap-3.5 overflow-hidden">
              ...
              <div className="bg-gray-50/50 p-2 rounded-lg border border-gray-100 shrink-0">
                ...
                <div className="text-[8px] font-black text-gray-700 space-y-0.5">
                  <div className="flex gap-1 bg-white p-0.5 rounded border border-gray-100">
                    <select value={info.day} onChange={e => setInfo({...info, day: e.target.value})} className="bg-transparent border-none p-0 focus:ring-0 appearance-none flex-grow text-center text-[11px] font-black">{DAYS.map(d => <option key={d} value={d}>{d}</option>)}</select>
                    <select value={info.month} onChange={e => setInfo({...info, month: e.target.value})} className="bg-transparent border-none p-0 focus:ring-0 appearance-none flex-grow text-center text-[11px] font-black">{MONTHS.map(m => <option key={m} value={m}>{m}</option>)}</select>
                    <select value={info.date} onChange={e => setInfo({...info, date: e.target.value})} className="bg-transparent border-none p-0 focus:ring-0 appearance-none flex-grow text-center text-[11px] font-black">{DATES.map(d => <option key={d} value={d}>{d}</option>)}</select>
                  </div>
                  <div className="flex items-center justify-center bg-[#772432] text-white py-0.5 rounded shadow-sm">
                    <input className="bg-transparent outline-none w-16 font-black text-center text-xl leading-none" value={info.time} onChange={e => setInfo({...info, time: e.target.value})} />
                  </div>
                </div>
              ...
              <div className="bg-[#004165] text-white px-2 py-1 rounded-lg border-b-2 border-[#F2DF74] shrink-0">
                 ...
                 <input className="w-full bg-transparent outline-none text-lg font-black text-[#F2DF74] italic uppercase tracking-[0.05em] text-center leading-none" value={info.wordOfTheDay} onChange={e => setInfo({...info, wordOfTheDay: e.target.value})} />
              </div>

              {/* 3. Time Rule */}
              <div className="border border-blue-50 rounded-lg overflow-hidden shrink-0 bg-white">
                <div className="bg-[#e1f5fe]/50 px-2 py-0.5 border-b border-blue-100">
                  <h3 className="font-bold italic text-[11px] text-[#004165] leading-none text-center">Time Rule</h3>
                </div>
                <table className="w-full text-left text-[9px] font-bold border-collapse">
                  <thead>
                    <tr className="border-b border-gray-50 bg-gray-50/20">
                      ...
                      <th className="px-1 py-0.5 text-[#004165]">S</th>
                      <th className="px-1 py-0.5 text-[#004165]">L</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="border-b border-gray-50">
                      <td className="bg-[#ccff00] px-2 py-0.5 leading-none text-black">Green</td>
                      <td className="px-1 py-0.5 leading-none">1m</td><td className="px-1 py-0.5 leading-none">2m</td>
                    </tr>
                    <tr className="border-b border-gray-50">
                      <td className="bg-yellow-400 px-2 py-0.5 leading-none text-black">Yellow</td>
                      <td className="px-1 py-0.5 leading-none">0.5m</td><td className="px-1 py-0.5 leading-none">1m</td>
                    </tr>
                    <tr className="border-b border-gray-50">
                      <td className="bg-red-600 text-white px-2 py-0.5 leading-none">Red</td>
                      <td className="px-1 py-0.5 leading-none">Up</td><td className="px-1 py-0.5 leading-none">Up</td>
                    </tr>
                    <tr>
                      <td className="bg-gray-50 px-2 py-0.5 text-gray-500 leading-none">Grace</td>
                      <td className="px-1 py-0.5 leading-none" colSpan={2}>30 seconds</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              {/* 4. Reminder */}
              <div className="border border-red-50 rounded-lg overflow-hidden shrink-0 flex flex-col bg-white">
                <div className="bg-[#fce4ec]/50 px-2 py-0.5 border-b border-red-100">
                  <h3 className="font-bold italic text-[11px] text-[#772432] leading-none text-center">Reminder</h3>
                </div>
                <div className="p-1.5 py-1 space-y-0.5 text-[9px] font-bold text-gray-800 leading-tight">
                  {reminders.map((r, idx) => (
              ...
              <div className="mt-1 border border-amber-50 rounded-lg overflow-hidden shrink-0 bg-white shadow-sm">
                ...
                <div className="p-1.5 py-1 text-[9px] font-bold text-gray-800 leading-tight text-center">
                  Free for members, 38 RMB for visiting non-members.<br/>
                  <span className="text-[#772432] font-bold">(Free for first-time attendees)</span>
                </div>
              </div>
          ...
          {/* BOTTOM BRANDING - OPTIMIZED SPACING */}
